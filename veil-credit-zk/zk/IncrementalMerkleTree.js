...
export class IncrementalMerkleTree {
  constructor(bb) {
    this.bb = bb;
    this.nextIndex = 0;
    this.cached = Array(DEPTH).fill(null);
    this.root = ZERO_VALUES[DEPTH - 1];
    this.indexOf = new Map(); // commitment -> index
  }

  async insert(commitmentStr) {
    let idx = this.nextIndex;
    let current = new Fr(BigInt(commitmentStr));

    this.indexOf.set(commitmentStr, idx);
    ...
    for (let level = 0; level < DEPTH; level++) {
      ...
      if (idx % 2 === 0) {
        left = current;
        right = new Fr(BigInt(ZERO_VALUES[level]));
        this.cached[level] = current;
      } else {
        left = this.cached[level];
        right = current;
      }

      current = await this.bb.poseidon2Hash([left, right]);
      idx >>= 1;
    }

    this.root = current.toString();
    this.nextIndex++;
  }

  async getProof(commitmentStr) {
    const index = this.indexOf.get(commitmentStr);
    if (index === undefined) {
      throw new Error("Commitment not in deposit tree");
    }
    ...
    for (let level = 0; level < DEPTH; level++) {
      if (idx % 2 === 0) {
        ...
        pathIndices.push(true);
      } else {
        pathElements.push(this.cached[level].toString());
